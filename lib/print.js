...
console.log(`${chalk.cyanBright('┃')} ${chalk.redBright('Bot:')} ${me} ~${conn.user.name}${conn.user.jid == global.conn.user.jid ? '' : ' (𝗦𝗨𝗕 𝗕𝗢𝗧)'}
${chalk.cyanBright('┃')}
${chalk.cyanBright('┃')} ${chalk.yellow('Fecha:')} ${(m.messageTimestamp ? new Date(1000 * (m.messageTimestamp.low || m.messageTimestamp)) : new Date).toTimeString()}
${chalk.cyanBright('┃')}
${chalk.cyanBright('┃')} ${chalk.green('Tipo de evento:')} ${m.messageStubType ? WAMessageStubType[m.messageStubType] : 'Ninguno'}
${chalk.cyanBright('┃')}
${chalk.cyanBright('┃')} ${chalk.magenta('Peso del mensaje:')} ${filesize} B [${filesize === 0 ? 0 : (filesize / 1000 ** Math.floor(Math.log(filesize) / Math.log(1000))).toFixed(1)} ${['B', 'KB', 'MB', 'GB', 'TB'][Math.floor(Math.log(filesize) / Math.log(1000))] || ''}]
${chalk.cyanBright('┃')}
${chalk.cyanBright('┃')} ${chalk.blueBright('Remitente:')} ${sender}
${chalk.cyanBright('┃')}
${chalk.cyanBright('┃')} ${chalk.yellow('Usuario:')} ${user ? '|' + user.exp + '|' + user.money : '' + ('|' + user.level + user.limit)}
${chalk.cyanBright('┃')}
${chalk.cyanBright('┃')} ${chalk.cyan('Chat:')} ${m.chat}${chat ? ' ~' + chat : ''}
${chalk.cyanBright('┃')}
${chalk.cyanBright('┃')} ${chalk.magenta('Tipo de mensaje:')} ${m.mtype ? m.mtype.replace(/message$/i, '').replace('audio', m.msg?.ptt ? 'PTT' : 'audio').replace(/^./, v => v.toUpperCase()) : 'Desconocido'}
${chalk.cyanBright('┃')}
`)
console.log(chalk.cyanBright('┕━━━━ ⋆⋅☆⋅⋆ ━━━━┙'))

...
console.log(`${chalk.cyanBright('┃')} ${chalk.redBright('Bot:')} ${chalk.whiteBright(me)} ~${chalk.magentaBright(conn.user.name)}${conn.user.jid == global.conn.user.jid ? '' : chalk.red(' (𝗦𝗨𝗕 𝗕𝗢𝗧)')}
${chalk.cyanBright('┃')}
${chalk.cyanBright('┃')} ${chalk.yellow('Fecha:')} ${chalk.greenBright((m.messageTimestamp ? new Date(1000 * (m.messageTimestamp.low || m.messageTimestamp)) : new Date).toTimeString())}
${chalk.cyanBright('┃')}
${chalk.cyanBright('┃')} ${chalk.green('Tipo de evento:')} ${chalk.blueBright(m.messageStubType ? WAMessageStubType[m.messageStubType] : 'Ninguno')}
${chalk.cyanBright('┃')}
${chalk.cyanBright('┃')} ${chalk.magenta('Peso del mensaje:')} ${chalk.yellowBright(filesize + ' B')} [${chalk.greenBright(filesize === 0 ? 0 : (filesize / 1000 ** Math.floor(Math.log(filesize) / Math.log(1000))).toFixed(1))} ${chalk.blueBright(['B', 'KB', 'MB', 'GB', 'TB'][Math.floor(Math.log(filesize) / Math.log(1000))] || '')}]
${chalk.cyanBright('┃')}
${chalk.cyanBright('┃')} ${chalk.blue('Remitente:')} ${chalk.whiteBright(sender)}
${chalk.cyanBright('┃')}
${chalk.cyanBright('┃')} ${chalk.yellow('Usuario:')} ${chalk.magentaBright(user ? '|' + user.exp + '|' + user.money : '' + ('|' + user.level + user.limit))}
${chalk.cyanBright('┃')}
${chalk.cyanBright('┃')} ${chalk.cyan('Chat:')} ${chalk.whiteBright(m.chat)}${chat ? chalk.gray(' ~' + chat) : ''}
${chalk.cyanBright('┃')}
${chalk.cyanBright('┃')} ${chalk.magenta('Tipo de mensaje:')} ${chalk.green(m.mtype ? m.mtype.replace(/message$/i, '').replace('audio', m.msg?.ptt ? 'PTT' : 'audio').replace(/^./, v => v.toUpperCase()) : 'Desconocido')}
${chalk.cyanBright('┃')}
`)
console.log(chalk.cyanBright('┕━━━━ ⋆⋅☆⋅⋆ ━━━━┙'))

...
console.log(`${chalk.redBright('Bot:')} ${me} ~${conn.user.name}${conn.user.jid == global.conn.user.jid ? '' : ' (𝗦𝗨𝗕 𝗕𝗢𝗧)'}
${chalk.yellow('Fecha:')} ${(m.messageTimestamp ? new Date(1000 * (m.messageTimestamp.low || m.messageTimestamp)) : new Date).toTimeString()}
${chalk.green('Tipo de evento:')} ${m.messageStubType ? WAMessageStubType[m.messageStubType] : 'Ninguno'}
${chalk.magenta('Peso del mensaje:')} ${filesize} B [${filesize === 0 ? 0 : (filesize / 1000 ** Math.floor(Math.log(filesize) / Math.log(1000))).toFixed(1)} ${['B', 'KB', 'MB', 'GB', 'TB'][Math.floor(Math.log(filesize) / Math.log(1000))] || ''}]
${chalk.blueBright('Remitente:')} ${sender}
${chalk.green('Exp ganada:')} ${m?.exp ?? '?'}
${chalk.yellow('Usuario:')} ${user ? '|' + user.exp + '|' + user.money : '' + ('|' + user.level + user.limit)}
${chalk.cyan('Chat:')} ${m.chat}${chat ? ' ~' + chat : ''}
${chalk.magenta('Tipo de mensaje:')} ${m.mtype ? m.mtype.replace(/message$/i, '').replace('audio', m.msg?.ptt ? 'PTT' : 'audio').replace(/^./, v => v.toUpperCase()) : 'Desconocido'}`)
console.log(chalk.cyanBright('┕━━━━ ⋆⋅☆⋅⋆ ━━━━┙'))

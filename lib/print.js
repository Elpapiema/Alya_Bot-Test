...
export default async function (m, conn = { user: {} }) {
  let _name = await conn.getName(m.sender)
  let sender = new PhoneNumber('+' + m.sender.replace('@s.whatsapp.net', '')).getNumber('international') + (_name ? ' ~' + _name : '')
  let chat = await conn.getName(m.chat)
  let img
  ...
  let user = global.db.data.users[m.sender]
  let me = new PhoneNumber('+' + (conn.user?.jid).replace('@s.whatsapp.net', '')).getNumber('international')
 ...
 ${chalk.green('%s')} ${chalk.yellow('%s%s')} ${chalk.blueBright('-')} ${chalk.green('%s')} ${chalk.cyanBright(chalk.blueBright('%s'))}`.trim(),
    ...
    ['','K','M','G','T','P'][Math.floor(Math.log(filesize) / Math.log(1000))] || '',
    sender,
    m ? m.exp : '?',
    user ? '|' + user.exp + '|' + user.money : '' + ('|' + user.level + user.limit ),
    m.chat + (chat ? ' ~' + chat : ''),
  ...
  if (m.messageStubParameters) {
    console.log(m.messageStubParameters.map(jid => {
      jid = conn.decodeJid(jid)
      let name = conn.getName(jid)
      return chalk.gray(new PhoneNumber('+' + jid.replace('@s.whatsapp.net', '')).getNumber('international') + (name ? ' ~' + name : ''))
    }).join(', '))
  }
  ...
  if (typeof m.text === 'string' && m.text) {
    ...
    if (m.mentionedJid)
      for (let jid of m.mentionedJid)
        log = log.replace('@' + jid.split`@`[0], chalk.blueBright('@' + await conn.getName(jid)))

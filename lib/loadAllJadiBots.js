...
export default async function loadAllJadiBots() {
	const authJadi = path.join(__dirname, "../../Alya-SubBots");
	const sesions = await fs.readdir(authJadi);
	if (sesions.length !== 0) {
        console.log("Cargando " + sesions.length + " Sesiones de la carpeta 'Alya-SubBots'"); // que asco tiene 'bts' :v

...
let handler = async (m, { conn, text, usedPrefix, command }) => {
  try {
    ...
    // ✅ Aplanar servers (viene como array de arrays)
    const serversList = Array.isArray(data.servers?.[0])
      ? data.servers[0]
      : data.servers;

    if (!serversList || !serversList.length) {
      throw new Error('⚠️ No se encontraron servidores disponibles');
    }

    const megaServer = serversList.find(s => s.server === 'mega');
    if (!megaServer || !megaServer.code) {
      throw new Error('⚠️ No se encontró el enlace de MEGA');
    }

    // 🩹 ===== PARCHE MEGA =====
    let megaUrl = megaServer.code.trim();

    // Formato esperado por la API: https://mega.nz/embed/!ID!KEY
    if (megaUrl.includes('/embed/')) {
      const parts = megaUrl.split('!');
      if (parts.length >= 3) {
        const id = parts[1];
        const key = parts[2];
        megaUrl = `https://mega.nz/file/${id}#${key}`;
      }
    }

    // Validar que tenga hash
    if (!megaUrl.includes('#')) {
      throw new Error('⚠️ Enlace de MEGA inválido: no contiene clave (#)');
    }
    // 🩹 ======================
...
let handler = async (m, { conn, text, usedPrefix, command }) => {
  try {
    ...
    if (megaUrl.includes('mega.nz/embed/')) {
      megaUrl = megaUrl.replace('mega.nz/embed/', 'mega.nz/file/');
    }
